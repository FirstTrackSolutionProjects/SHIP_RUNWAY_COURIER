  ...
  return (
    ...
    <div>
      ...
      <button
        ...
        className="md:hidden p-3 absolute text-yellow-500 z-40"
      >
       ...
       <div
        className={`${isSidebarHovered ? 'w-[260px] min-w-[260px]' : 'w-[72px] min-w-[72px]'} md:block hidden h-full relative bg-[#0a0a0a] overflow-y-auto overflow-x-hidden transition-all duration-[350ms] ease-in-out shadow-2xl border-r border-yellow-500/10`}
      ...
      <ul className="p-3 space-y-2">
       ...
       <div
        className={`relative ${isOpen?'w-[290px]': 'w-0'} block md:hidden h-full bg-[#0a0a0a] overflow-y-auto overflow-x-hidden transition-all duration-300 border-r border-yellow-500/20`}
      >
        {/* Close button (Icon) */}
        <ul className="p-4 pt-14 space-y-3">
        {sidebarItems.map((item) => {
          if ((item.admin && !admin) || (item.merchantOnly && admin)) {
            return;
          }
